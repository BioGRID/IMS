...
class User {
	...
	private function updatePersistent( $userID, $setCookie = false ) {
		...
		$stmt = $this->db->prepare( "UPDATE " . DB_IMS . ".users SET user_uuid=?, user_random=?, user_lastlogin=NOW( ) WHERE user_id=?" );
	...
	private function verifyPersistent( $uuid, $random ) {
		...
		$stmt = $this->db->prepare( "SELECT user_id FROM " . DB_IMS . ".users WHERE user_uuid=? AND user_random=? AND user_status='active'" );
	...
	private function verifyPassword( $username, $password ) {
		...
		$stmt = $this->db->prepare( "SELECT user_id, user_password FROM " . DB_IMS . ".users WHERE user_name=? AND user_status='active'" );
	...
	public function delUser( $userID ) {
		...
		$stmt = $this->db->prepare( "UPDATE " . DB_IMS . ".users SET user_status='inactive' WHERE user_id=?" );
		$stmt->execute( array( $userID ) );
		$stmt->close( );
	...
	public function changePassword( $userID, $password ) {
		...
		$stmt = $this->db->prepare( "UPDATE " . DB_IMS . ".users SET user_password=? WHERE user_id=?" );
	...
	public function addUser( $userName, $password, $firstname, $lastname, $email, $class ) {
		...
		$stmt = $this->db->prepare( "SELECT user_id FROM " . DB_NAME . ".users WHERE user_name=? OR user_email=?" );
		...
		$passwordHash = $this->generatePasswordHash( $password );
		$stmt = $this->db->prepare( "INSERT INTO " . DB_NAME . ".users VALUES( 0,?,?,?,?,?,'0','-','0000-00-00 00:00:00',?,'active','1' )" );
	...
	public function fetchUsersList( $fetchInactive = false ) {
		...
		$stmt = $this->db->prepare( "SELECT user_id, user_name, user_firstname, user_lastname, user_email, user_lastlogin, user_class, user_status FROM " . DB_IMS . ".users WHERE user_id != '1' ORDER BY user_firstname ASC" );
		$stmt->execute( );
	...
	public function usernameExists( $username ) {
		...
		$stmt = $this->db->prepare( "SELECT user_id FROM " . DB_IMS . ".users WHERE user_name=? LIMIT 1" );
		$stmt->execute( array( $username ) );
	...
	public function emailExists( $email ) {
		...
		$stmt = $this->db->prepare( "SELECT user_id FROM " . DB_IMS . ".users WHERE user_email=? LIMIT 1" );
	...
	public function changeUserLevel( $userID, $levelDirection ) {
		...
		$stmt = $this->db->prepare( "SELECT user_class FROM " . DB_IMS . ".users WHERE user_id=? LIMIT 1" );
		$stmt->execute( $userID );
		...
		$stmt = $this->db->prepare( "UPDATE " . DB_IMS . ".users SET user_class=?, user_uuid='-', user_random='0' WHERE user_id=?" );
	...
	public function changeUserStatus( $userID, $status ) {
		...
		$stmt = $this->db->prepare( "UPDATE " . DB_IMS . ".users SET user_status=?, user_uuid='-', user_random='0' WHERE user_id=?" );
